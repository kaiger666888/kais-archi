...
export async function generateAll(targetDir, options = {}) {
  ...
  for (const item of mermaidFiles) {
    const code = fs.readFileSync(path.join(outDir, item.mmdFile), 'utf-8');
    const isDup = deduped.some(d => {
      const dCode = fs.readFileSync(path.join(outDir, d.mmdFile), 'utf-8');
  ...
  return { outDir, pages: pages.filter(p => !p.error), mermaidFiles: finalMermaidFiles, notionPages };
}
