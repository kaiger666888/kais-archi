...
function writeDiagramToPage(pageId, options) {
  ...
  if (split) {
    appendParagraph(pageId, `${label || type} — 拆分为 ${parts.length} 个子图`);
  }

  const imageUrls = [];
  for (let i = 0; i < codeBlocks.length; i++) {
    const code = codeBlocks[i];
    const r = mmdcRenderToPng(code);
    if (!r.valid) {
      console.log(`    ❌ ${label} part${split ? (i+1) : ''}: 渲染失败 — ${r.error}`);
    ...
    // 上传 PNG 到 GitHub
    const fileLabel = `${label.replace(/[^a-zA-Z0-9]/g, '-')}-${i + 1}`;
    const imgUrl = uploadPngToGithub(r.pngPath, fileLabel);
    imageUrls.push(imgUrl);
    console.log(`    ✅ ${label} part${split ? (i+1) : ''} — 渲染并上传 (${imgUrl.split('/').slice(-1)})`);
  }
  ...
  for (const url of imageUrls) {
    ...
    writeFileSync(jsonFile, JSON.stringify({
      children: [{ type: 'image', image: { type: 'external', external: { url } } }],
    }));
  ...
  // 源码 callout（折叠）
  appendDivider(pageId);
  appendCallout(pageId, '📝', `Mermaid 源码 (${originalLen} 字符${compressed ? '，已压缩' : ''})`);
  for (const code of codeBlocks) {
    appendCodeBlock(pageId, code, 'mermaid');
  }
  appendDivider(pageId);
  return { written: true };
}
