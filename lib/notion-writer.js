...
function writeDiagramToPage(pageId, options) {
  ...
  const { code: notionCode, compressed, originalLen, truncated } = compressMermaid(rawCode);
  appendHeading2(pageId, `📊 ${label || type}`);
  if (truncated) {
    appendCallout(pageId, '⚠️', `图表过大(${(originalLen/1024).toFixed(1)}KB)已截断，完整版见 .mmd 文件`);
  } else if (compressed) {
    appendParagraph(pageId, `${label || type} — 已压缩(${originalLen}→${notionCode.length}字符)`);
  }
  appendCodeBlock(pageId, notionCode, 'mermaid');
