...
/**
 * 从步骤描述中提取简洁的 I/O 标注
 * "读取每个 VideoShot 的 api_params" → "📥 VideoShot"
 * "返回 VideoClip" → "📤 VideoClip"
 * "调用 executeShot" → "⚙️ executeShot"
 */
function extractIOAnnotation(text) {
  ...
  // 调用模式（只在没有输入时才显示，避免重复）
  if (parts.length === 0) {
    const callMatch = text.match(/(?:调用|使用|执行)\s*[`'"]?(\w+)[`'"]?/);
    if (callMatch) {
      parts.push(`⚙️${callMatch[1]}`);
    }
  }
