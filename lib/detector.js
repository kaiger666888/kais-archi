...
export async function detectArchitecture(targetDir) {
  const [phases, skills, libraries, dataFlow, crossCutting] = await Promise.all([
...
export async function detectPipeline(targetDir) {
  ...
  while ((match = phaseRegex.exec(skillMd)) !== null) {
    ...
    // 检查是否有 git checkpoint
    const lineEnd = skillMd.slice(match.index).split('\n')[0];
    const hasGit = lineEnd.includes('git checkpoint') || lineEnd.includes('📌');

    // 提取标签（Phase 后面的 tag 文字）
    const descLine = skillMd.slice(match.index).split('\n')[1] || '';
    const tags = extractTags(descLine);

    // 检查是否有 FAIL/回滚
    const nextLines = skillMd.slice(match.index).slice(0, 300);
...
export async function detectDataFlow(targetDir) {
  ...
  while ((match = flowRegex.exec(skillMd)) !== null) {
    ...
    if (/Phase|Step|mode/.test(from) && !/\.json|\.png|\.mp4/.test(from)) continue;

    flows.push({ from, to });
  }

  // 也匹配共享工具表中的文件
  const toolTableMatch = skillMd.match(/## 共享工具[\s\S]*?(?=##|$)/);
  if (toolTableMatch) {
    const toolRegex = /\|\s*(\S+\.\w+)\s*\|/g;
    while ((match = toolRegex.exec(toolTableMatch[0])) !== null) {
      if (!flows.find(f => f.from === match[1])) {
        flows.push({ from: match[1], to: '共享工具' });
      }
    }
  }
