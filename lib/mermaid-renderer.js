...
function pipelineToMermaid(model) {
  ...
  for (let i = 0; i < phases.length; i++) {
    ...
    const label = safeLabel(`${p.id}: ${p.name}${gitMarker}`);
...
function callGraphToMermaid(model) {
  ...
  for (const [g, gNodes] of groups) {
    ...
    if (gNodes.length <= MAX_NODES_PER_SUB) {
      ...
      // 拆分
      const chunks = [];
      for (let i = 0; i < gNodes.length; i += MAX_NODES_PER_SUB) {
        chunks.push(gNodes.slice(i, i + MAX_NODES_PER_SUB));
      }
      const suffixes = ['①', '②', '③', '④', '⑤'];
      for (let i = 0; i < chunks.length; i++) {
        const subId = `${safeId(g)}_${i}`;
        const suffix = chunks.length <= 3 ? suffixes[i] : `-${i + 1}`;
        lines.push(`    subgraph ${subId}["${label}${suffix}"]`);
        lines.push(`    style ${subId} fill:${color}22,stroke:${color},stroke-width:2px`);
        for (const node of chunks[i]) {
...
function layerMapToMermaid(model) {
  ...
  const MAX_NODES_PER_SUB = 12; // 每 subgraph 最多 12 个节点，避免过宽
  ...
  for (const layer of layers) {
    ...
    if (items.length <= MAX_NODES_PER_SUB) {
      // 不需要拆分
      lines.push(`    subgraph ${id}["${layer.name}"]`);
      lines.push(`    style ${id} fill:${color}22,stroke:${color},stroke-width:2px`);
      for (const item of items) {
        lines.push(`        ${safeId(item.id)}("${safeLabel(item.label)}")`);
      }
      lines.push('    end');
      lines.push('');
    } else {
      // 拆分为多个子 subgraph：专业能力层-1, 专业能力层-2, ...
      const chunks = [];
      for (let i = 0; i < items.length; i += MAX_NODES_PER_SUB) {
        chunks.push(items.slice(i, i + MAX_NODES_PER_SUB));
      }
      if (chunks.length <= 3) {
        // 3 个以内：直接用 A/B/C 后缀
        const suffixes = ['①', '②', '③'];
        for (let i = 0; i < chunks.length; i++) {
          const subId = `${id}_${i}`;
          const label = `${layer.name}${suffixes[i]}`;
